    ...
    getInvoiceList(request) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const invoicescountQry = "SELECT count(*) AS total_count FROM invoices WHERE 1 = 1 " + cond;
                const totalLenResp = yield db_1.db.selectRow(invoicescountQry);
                ...
                if (select == "")
                    select = "*";
                const invoiceQry = `SELECT ${select} FROM invoices
          WHERE 1 = 1 ` +
                    cond +
                    orderby +
                    limit;
                let result = yield db_1.db.select(invoiceQry);
    ...
    getContractorInvoices(request) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const invoicescountQry = "SELECT count(*) AS total_count FROM contractors_invoices ci LEFT JOIN invoices i ON ci.invoiceId=i.id WHERE 1 = 1 " + cond;
                const totalLenResp = yield db_1.db.selectRow(invoicescountQry);
                recordsTotal = totalLenResp.total_count;
                let select = "";
                if (columns && global != undefined && global == false) {
                ...
                if (select == "")
                    select = "*";
                const invoiceQry = `SELECT ${select} FROM contractors_invoices ci LEFT JOIN invoices i ON ci.invoiceId=i.id WHERE 1 = 1 ` +
                    cond +
                    orderby +
                    limit;
                let result = yield db_1.db.select(invoiceQry);
    ...
    getInvoiceItems(request) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const invoiceItemsCountQry = "SELECT count(*) AS total_count FROM invoice_items WHERE 1 = 1 " + cond;
                const totalLenResp = yield db_1.db.selectRow(invoiceItemsCountQry);
                recordsTotal = totalLenResp.total_count;
                let select = "";
                if (columns && global != undefined && global == false) {
                    columns.forEach((item, index) => {
                ...
                if (select == "")
                    select = "*";
                const invoiceItemsQry = `SELECT ${select} FROM invoice_items
        WHERE 1 = 1 ` +
                    cond +
                    orderby +
                    limit;
                let result = yield db_1.db.select(invoiceItemsQry);
                this.response.status = "ok";
                this.response.draw = draw;
                this.response.recordsTotal = recordsTotal;
                this.response.recordsFiltered = recordsTotal;
...
exports.invoiceModelObj = new invoiceModel();
