...
class orderModel {
  response: any = {};
  user: any = {};
  async getOrderList(request: any) {
    this.response = {};
    ...
    try {
      ordersResult = await db.select(ordersQry);
      const qry =
      ...
      for (i = 0; i < ordersResult.length; i++) {
        if (
          ordersResult[i].clientId &&
          account_managers[ordersResult[i].clientId]
        ) {
          ordersResult[i] = {
            ...ordersResult[i],
            account_managers: account_managers[ordersResult[i].clientId]
          };
        }
      }
    } catch (e) {
  ...
  async addedit(req: any) {
    ...
    if (orderId > 0) {
      const result = await this.editInformation(orderId, formData, fetchMethod);
      await historyServiceObj.logHistory(
        req.user,
        `edited the order ${orderId}`,
        "order",
        orderId
      );
      return result;
    } else {
      const result = await this.addInformation(formData, fetchMethod);
      await historyServiceObj.logHistory(
        req.user,
        `created the order ${result.newOrderId}`,
        "order",
        result.newOrderId
      );
      return result;
  ...
  async addInformation(formData: any, fetchMethod: number) {
    let now = new Date();
    ...
    if (fetchMethod == 0) {
      ...
      insertData = { ...sepcial_type, ...insertData };
    } else {
      let sepcial_type = { special_type: true };
      insertData = { ...sepcial_type, ...insertData };
    }
    const orderInsResp = await db.insert(insertQry, insertData);
    ...
    if (formData.clientId) {
      ...
      await db.insert(insertClientOrderQry, [
    ...
    if (formData.contractorId) {
      ...
      await db.insert(insertContractorOrderQry, [
  ...
  async editInformation(orderId: any, formData: any, fetchMethod: number) {
    let now = new Date();
    let current_time = dateFormat(now, "yyyy-mm-dd HH:MM:ss");
    ...
    if (formData.clientId) {
      const clientsOrder = await db.selectRow(
        `SELECT * FROM clients_order WHERE orderId = ${orderId}`
      );
      if (clientsOrder.orderId == undefined) {
        ...
        await db.update(updateClientOrderQry, {
    ...
    if (formData.contractorId) {
      const contractorsOrder = await db.selectRow(
        `SELECT * FROM contractors_order WHERE orderId = ${orderId}`
      );
      if (contractorsOrder.orderId == undefined) {
        ...
        await db.update(updateContractorOrderQry, {
  ...
  async getOrderDetails(data: any) {
    ...
    const qry = `SELECT cam.id, cam.name, cam.phone
                  ...
                  ORDER BY cam.order`;
    let account_managers = await db.select(qry);
    ...
    let orderResult = await db.selectRow(orderQry);
    orderResult = { ...orderResult, account_managers };
    this.response.status = "ok";
  ...
  async getNewOrderNo() {
    ...
    let neworderId = 0;
    const ordersQry =
      "SELECT orderId FROM orders WHERE 1=1 ORDER BY  orderId DESC LIMIT 1  ";
    const ordersResult = await db.selectRow(ordersQry);
  ...
  async getOpenPositions(data: any) {
    ...
    const totalLenResp = await db.selectRow(countQry);
    ...
    try {
      const ordersResult = await db.select(ordersQry);
      this.response.status = "ok";
      this.response.data = ordersResult;
      return {
        recordsTotal,
  ...
  async savePriceHistory(req_data: any) {
    try {
      let insert: any = req_data.insert;
      let update: any = req_data.update;
      let orderId: number = req_data.orderId;
      let deletedIds: any = req_data.deletedIds;
      let entityId: number = req_data.entityId;
      let priceType: string = req_data.priceType;

      let insertData = insert.map((x: any) => {
        return [
          orderId,
          entityId,
          x.price,
  ...
  async getPriceHistory(orderId: number, entityId: number, priceType: string) {
    try {
      let condition = `priceType = '${priceType}'`;
      if (entityId) condition = `${condition} AND entityId = ${entityId}`;
      if (orderId) condition = `${condition} AND orderId = ${orderId}`;
      let qry = `SELECT * FROM price_history WHERE ${condition} ORDER BY valid_from DESC`;
      let history = await db.select(qry);
      return history;
    } catch (e) {
      throw e;
    }
...
export let orderModelObj: any = new orderModel();
