...
class personModel {
  response: any = {};
  async getPersonList(request: any) {
    this.response = {};
    ...
    const personcountQry =
      "SELECT count(*) AS total_count FROM persons WHERE status = 1 " + cond;
    ...
    if (select == "") select = "*";
    const personQry =
      "SELECT * FROM persons WHERE status = 1 " + cond + orderby + limit;
  ...
  async addedit(req: any) {
    ...
    let result;
    if (personsId > 0) {
      result = await this.editInformation(personsId, formData);
      await historyServiceObj.logHistory(
        req.user,
        `edited the client ${personsId}`,
        "person",
        personsId
      );
    } else {
      ...
      await historyServiceObj.logHistory(
        req.user,
        `created the client ${result.newPersonId}`,
        "person",
        result.newPersonId
      );
    }
  ...
  async addInformation(formData: any) {
    try {
      ...
      const personInsResp = await db.insert(insertQry, insertData);
  ...
  async editInformation(personsId: any, formData: any) {
    try {
      let now = new Date();
      let current_time = dateFormat(now, "yyyy-mm-dd HH:MM:ss");
  ...
  async getPersonDetails(data: any) {
    let resultData: any = {};
    let personsId = parseInt(data.personId);

    const personQry =
      "SELECT * FROM persons WHERE status = ? AND personId = ? ";
    const personResult = await db.selectRow(personQry, [1, personsId]);
  ...
  async deletePerson(data: any) {
    ...
    const result = await db.delete(deletePersonQry);
    this.response.status = "ok";
    this.response.data = {
  ...
  async getNewPersonsNo() {
    this.response = {};
    let newpersonsId = 0;
    const personsQry =
      "SELECT personId FROM persons WHERE 1=1 ORDER BY  personId DESC LIMIT 1  ";
    const personsResult = await db.selectRow(personsQry);
  ...
  async personOrderList(request: any) {
    this.response = {};
    let req_data = request.body;
    ...
    if (newOrderIds && newOrderIds.length > 0) {
      ...
      else firstCond += ` AND ( o.orderId IN(${newPIds}))`;
    } else {
    ...
    if (status == 1) firstCond += " AND o.status = 1 ";
    if (status == 0) firstCond += " AND o.status = 0 ";
...
export let personModelObj: any = new personModel();
